...
![The whole WISP machine: a fiber POP feeds a backhaul link to a high site, whose sectors reach rooftop CPE at customer homes, who pay monthly.](../../../assets/diagrams/wisp-machine.svg)
...
- **The implication:** there's a customer count below which you lose money
  ...
  and here's a starting version of it. Defaults are this page's mid-2026
  ballparks; replace every one with your market's reality:
...
**Run the checks — this is an afternoon, address by address.** Pick ten
sample addresses spread across your target area and check each against all
three competitors: the [Starlink availability map](https://www.starlink.com/map)
for satellite, each carrier's home-internet address checker for FWA, and the
[FCC National Broadband Map](https://broadbandmap.fcc.gov/) for what every
provider *claims* at that location (remember it's self-reported — Stage 2
explains why — so treat it as the optimistic ceiling, then ground-truth by
asking actual residents). For the subsidy threat, your **state broadband
office publishes its BEAD award maps** — find yours via the
[NTIA's data hub](https://broadbandusa.ntia.gov/resources/data-and-mapping)
and check whether funded fiber is coming to your exact footprint.
...
**The desk method, concretely.** All four checks run from a laptop before you
spend a dollar:

1. **Count rooftops** in Google Earth: drop a pin on each candidate high
   site, draw a 3–5 mile ring, and actually count structures — tedious for
   an hour, and it replaces a guess with a number you'll bet money on.
2. **Test the paths** with a free professional planner:
   [Cambium LINKPlanner](https://www.cambiumnetworks.com/products/software/linkplanner/)
   (desktop, pulls real elevation profiles automatically) or Ubiquiti's
   [UISP Design Center](https://ispdesign.ui.com/) (browser). Place your
   candidate site, place a dozen sample rooftops, and let the terrain tell
   you which neighborhoods are reachable — this is the same Fresnel/link
   math as [the Stage 1 calculators](/tools/), run against real ground.
3. **Read foliage at customer height**, not from above: Street View along
   the roads, looking back toward the high site. A canopy that looks thin
   in satellite view is a wall at rooftop height.
4. **Then drive it** — the desk pass tells you where to look; the drive
   tells you the truth. Note the high structures the satellite missed; every
   small town has a silo, tank, or steeple that doesn't show well from above.
...
**How to actually find the fiber:**

- **Your state broadband map** (via the
  [NTIA data hub](https://broadbandusa.ntia.gov/resources/data-and-mapping))
  and the FCC map show who claims fiber service near your area — every
  claimed business-fiber address implies glass in the ground nearby.
- **Ask the institutions**: the school district, library, hospital, and
  county offices almost certainly have fiber (in the US, E-Rate has funded
  school connectivity for decades) — and someone there knows which provider
  delivered it. That provider sells DIA.
- **Call the electric co-op.** Rural electric cooperatives increasingly run
  fiber along their own poles and sell wholesale transport — and they
  already know every structure in the county.
- **Get a real quote, not an assumption**: ask for *dedicated internet
  access* with an Ethernet handoff, at your candidate site's address —
  monthly cost, install cost, and construction cost if the fiber needs to be
  extended. Quote marketplaces can parallelize this across providers, but a
  direct call to the two or three regional fiber operators works fine.
...
## The Stage 0 exit checklist

"My area passes" is a feeling; these are artifacts. You're ready for Stage 1
when every one of them exists:

- [ ] **A rooftop count** — ~500+ structures within 3–5 miles of at least one
  candidate site, counted, not estimated
- [ ] **A candidate-site shortlist** with desk-checked paths to sample
  rooftops (the planner screenshots are your record)
- [ ] **A real DIA quote** at or near a candidate site — actual monthly,
  install, and construction numbers
- [ ] **A break-even number** from the calculator above, fed with *your*
  quote and lease numbers — comfortably under the ~50-per-site floor
- [ ] **The ten-address competitor check** done, and a one-sentence answer to
  "why would someone leave Starlink or carrier FWA for me?"
- [ ] **The BEAD award map checked** against your exact footprint
- [ ] **An interest list started**, with more names than your break-even
  number would eventually need at a 10% take rate
- [ ] **A survivable runway**: startup budget + 6 months of fixed costs you
  can lose without losing your house

Any box you can't check is Stage 0 homework, not a Stage 1 problem to defer.

## Where this goes next

With the checklist done — clustered demand, usable high sites, a fiber quote
in hand, a real advantage over the big three, and survivable runway — you're
ready for [Stage 1: First Customer](/journey/first-customer/): the RF
fundamentals you can't skip, your first site, your first link, and the moment
someone actually pays you.
